...
```
Where are we going to get the binary file to flash to eeprom? Well, the hexdump above shows what needs to be flashed to eeprom. One can create a binary file from the above listing using a hex editor or one might be able to get the file [here](https://github.com/larsenkb/larsenkb.github.io/img/ft2232hl_bob_new.bin).
